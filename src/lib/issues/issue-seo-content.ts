...
export function buildIssueSeoExtras(issue: Issue): IssueSeoExtras {
  ...
  const overview = [
    ...
    `برای پیشگیری از تکرار مشکل، ${formatList(issue.prevention, 4)} توصیه می‌شود. اگر پس از ریست یا بررسی‌های اولیه مشکل پابرجا بود، از صفحه ثبت سفارش تعمیر درخواست خود را با ذکر «${issue.title}» ثبت کنید تا کارشناسان ما هماهنگی لازم را انجام دهند. تمامی تعمیرات سخت‌افزاری شامل ضمانت تست ۳۰ روزه است. راهنمای کامل تعمیر این کنسول و مشکلات مرتبط را در کلاستر موضوعی همین صفحه ببینید.`,
  ];
