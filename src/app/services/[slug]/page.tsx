...
import ConsoleRepairLanding from "@/app/components/services/ConsoleRepairLanding";
...
import { getClusterByServiceSlug } from "@/lib/seo/topic-clusters";
...
export default async function ServicePage({ params }: Props) {
  ...
  const cluster = getClusterByServiceSlug(service.slug);
  const isConsolePillar = Boolean(consoleId && cluster);
  ...
  return (
    <main className="min-h-screen bg-black pt-24 text-white">
      ...
      >
        {isConsolePillar && consoleId && cluster ? (
          <ConsoleRepairLanding
            service={service}
            consoleId={consoleId}
            cluster={cluster}
          />
        ) : (
          <ClassicServiceLayout
            service={service}
            theme={theme}
...
function ClassicServiceLayout({
  ...
  theme: (typeof brandThemes)[keyof typeof brandThemes];
  repairHref: string;
  gallery: string[];
  consoleId: ReturnType<typeof consoleIdFromRepairSlug>;
}) {
  return (
    <>
