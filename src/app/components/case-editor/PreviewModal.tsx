...
export default function PreviewModal({ caseColor, caseMaterial, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-background/90 p-4">
      <div className="relative w-full max-w-sm">
        <button
          type="button"
          onClick={onClose}
          className="absolute -top-12 left-0 rounded-lg p-2 text-muted hover:text-foreground"
          aria-label="بستن"
        >
          <X size={24} />
        </button>
