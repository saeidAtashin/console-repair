    ...
    <div ref={containerRef} className={`relative ${className}`}>
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-white/10 bg-zinc-900/60 py-3.5 pl-4 pr-11 text-sm text-white placeholder:text-zinc-500 transition focus:border-cyan-500/50 focus:outline-none focus:ring-1 focus:ring-cyan-500/30"
        />
      </div>
      ...
      {showDropdown ? (
        <ul
          role="listbox"
          className="absolute z-20 mt-2 max-h-80 w-full overflow-y-auto rounded-2xl border border-white/10 bg-zinc-950/95 p-2 shadow-2xl backdrop-blur-xl"
        >
          {results.map((result) => (
            <li key={`${result.model.brandSlug}-${result.model.slug}`} role="option">
              ...
              >
                ...
                <div className="min-w-0 flex-1 text-right">
                  <p className="truncate text-sm font-bold text-white">{result.model.name}</p>
                  <p className="truncate text-xs text-zinc-500">{result.model.nameEn}</p>
                </div>
                <span className="shrink-0 rounded-lg bg-white/5 px-2 py-1 text-xs text-zinc-400">
                  {result.brand.name}
                </span>
              </Link>
            </li>
